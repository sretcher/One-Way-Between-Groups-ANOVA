...
Recall that Anova and Regression are both special cases of the general linear model, which is why we can use the same `lm` function for regression to find the residuals for ANOVA. We can then conduct the Shapiro–Wilk test on the model's residuals to test for normality.  Since the p-value is larger than our significance level(0.05), we do not reject the null hypothesis which means our k treatment populations are most likely normal.
